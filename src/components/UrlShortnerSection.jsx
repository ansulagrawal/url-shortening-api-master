...
function UrlShortnerSection() {
  ...
  const handleKeyDown = e => e?.key === "Enter" && handleOnClick();
    ...
    <div className="relative">
      <main className="flex flex-col md:flex-row sm:flex-row items-center gap-5 w-[80vw] m-auto xl:px-20 xl:pt-12 xl:pb-10 px-8 pt-6 pb-5 md:px-12 rounded-lg bg-dark-voilet bg-shorten-mobile md:bg-shorten-desktop bg-cover bg-no-repeat -mb-16">
        <input
          ...
          className="flex-1 bg-white w-full focus:ring-2 focus:ring-custom-cyan rounded border text-base outline-none text-gray-500 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
          placeholder="Shorten a link here..."
        />
        <button className="bg-custom-cyan w-full sm:w-auto border-0 py-2 px-6 focus:outline-none rounded text-white hover:bg-cyan-400" onClick={handleOnClick}>
          Shorten It!
        </button>
      </main>
    </div>
